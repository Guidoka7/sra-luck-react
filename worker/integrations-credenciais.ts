...
export const CATALOGO_PROVEDORES: Record<string, { nome: string; grupo: string; campos: CampoCredencial[] }> = {
  mercado_pago: { nome: "Mercado Pago", grupo: "pagamentos", campos: [
  ...
  conta_azul: { nome: "Conta Azul", grupo: "pagamentos", campos: [
  ...
  rd_station: { nome: "RD Station CRM", grupo: "crm", campos: [
  ...
  web_push: { nome: "Web Push", grupo: "comunicacao", campos: [
...
function campoDoProvedor(provedor: string, chave: string) { return CATALOGO_PROVEDORES[provedor]?.campos.find((campo) => campo.chave === chave) ?? null; }

async function derivarChave(segredo: string): Promise<CryptoKey> {
  const digest = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(`sra-luck:integrations:v1:${segredo}`));
  return crypto.subtle.importKey("raw", digest, { name: "AES-GCM" }, false, ["encrypt", "decrypt"]);
}
function paraBase64(bytes: Uint8Array) { let binario = ""; bytes.forEach((byte) => { binario += String.fromCharCode(byte); }); return btoa(binario); }
function deBase64(valor: string) { return Uint8Array.from(atob(valor), (caractere) => caractere.charCodeAt(0)); }
async function cifrarValor(segredo: string, valor: string) {
  const chave = await derivarChave(segredo); const iv = crypto.getRandomValues(new Uint8Array(12));
...
function mascarar(valor: string) { const limpo = valor.trim(); return limpo.length <= 4 ? "••••" : `•••• ${limpo.slice(-4)}`; }
async function tabelaDisponivel(db: ReturnType<typeof createServiceSupabaseClient>) { const { error } = await db.from("integracoes_credenciais").select("id", { count: "exact", head: true }); return !error; }
...
export async function obterCredencial(env: Env, provedor: string, chave: string): Promise<string | null> {
  ...
  try {
    ...
    const { data, error } = await db.from("integracoes_credenciais").select("valor_cifrado,valor_iv").eq("provedor", provedor).eq("chave", chave).eq("ativo", true).maybeSingle();
    if (error || !data) return doAmbiente;
    const valor = await decifrarValor(env.INTEGRATION_ENCRYPTION_KEY, data.valor_cifrado, data.valor_iv);
    return valor || doAmbiente;
  } catch { return doAmbiente; }
}

export async function salvarCredencialInterna(env: Env, provedor: string, chave: string, valor: string, actor: string) {
  if (!env.INTEGRATION_ENCRYPTION_KEY) throw new Error("CHAVE_CRIPTOGRAFIA_INTEGRACOES_AUSENTE");
  ...
  const { valorCifrado, iv } = await cifrarValor(env.INTEGRATION_ENCRYPTION_KEY, valor);
  const { error } = await db.from("integracoes_credenciais").upsert({ provedor, chave, valor_cifrado: valorCifrado, valor_iv: iv, valor_mascarado: mascarar(valor), ativo: true, atualizado_por: actor, atualizado_em: new Date().toISOString() }, { onConflict: "provedor,chave" });
...
export async function credenciaisApi(request: Request, env: Env): Promise<Response | null> {
  const url = new URL(request.url); if (url.pathname !== "/api/admin/integrations/credenciais") return null;
  const admin = await requireAdmin(request, env); if (!admin) return json({ erro: "Sessão administrativa expirada." }, 401);
  const colaborador = await buscarColaboradorAdminAtivo(admin, env).catch(() => null);
  if (!colaborador || !temPermissaoAdmin(colaborador, PERMISSOES_ADMIN.INTEGRACOES_GERENCIAR_CREDENCIAIS)) return json({ erro: "Seu papel não tem permissão para acessar credenciais de integrações." }, 403);
  const db = createServiceSupabaseClient(env);
  ...
  if (request.method === "GET") {
    ...
    return json({ provedores, persistenciaPronta: disponivel && Boolean(env.INTEGRATION_ENCRYPTION_KEY) });
  }

  if (request.method === "POST") {
    if (!env.INTEGRATION_ENCRYPTION_KEY) return json({ erro: "Cofre de integrações indisponível neste ambiente." }, 503);
    if (!(await tabelaDisponivel(db))) return json({ erro: "A estrutura para armazenar credenciais ainda não foi aplicada neste ambiente." }, 409);
    const body = await request.json().catch(() => ({})) as { provedor?: string; chave?: string; valor?: string; remover?: boolean };
    const provedor = String(body.provedor || ""); const chave = String(body.chave || ""); const campo = campoDoProvedor(provedor, chave);
    ...
    try { await salvarCredencialInterna(env, provedor, chave, valor, String(admin)); }
    catch { return json({ erro: "Não foi possível salvar a credencial." }, 500); }
