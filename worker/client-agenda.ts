...
export async function agenda(request: Request, env: Env): Promise<Response> {
  ...
  const { data: cliente, error: clienteError } = await supabase.from("clientes")
    ...
    .eq("id", s.clienteId).single();
  if (clienteError || !cliente) return json({ erro: "Cliente não encontrada." }, 404);

  const { data: agendamentos, error: agendaError } = await supabase.from("agendamentos")
    .select("id,data_id,status,horario_termos,termos_assinados_em,previsao_liberacao_financeira,data_cirurgia,created_at,datas(data)")
    .eq("cliente_id", cliente.id)
    .in("status", ["confirmado", "realizado"])
    .order("created_at", { ascending: false });
  if (agendaError) return json({ erro: "Não foi possível carregar sua agenda agora." }, 503);
  ...
  const limiteCirurgico = calcularLiberacaoCirurgica((termosConfirmados as any)?.termos_assinados_em ?? null, cliente.custeio_confirmado_em ?? null);
  ...
  const { data: remarcacoes } = agendamentoId
    ? await supabase.from("solicitacoes_remarcacao_agendamento")
      ...
      .eq("cliente_id", cliente.id).eq("agendamento_id", agendamentoId).order("created_at", { ascending: false })
    : { data: [] as any[] };
  ...
  // 90 dias é teto, nunca espera mínima. Depois de termos + quitação, qualquer
  // data administrativamente disponibilizada entre hoje e o teto pode ser escolhida.
  const elegivelCirurgia = Boolean((termosConfirmados as any)?.termos_assinados_em && cliente.custeio_confirmado_em && limiteCirurgico);
  let datasCirurgiaDisponiveis: Array<{ id: string; data: string; vagasRestantes: number }> = [];
  if (elegivelCirurgia && limiteCirurgico && limiteCirurgico >= hoje) {
    const [{ data: datasCirurgia }, { data: cirurgias }] = await Promise.all([
      supabase.from("datas_liberacao_financeira").select("id,data,status").eq("status", "disponivel").gte("data", hoje).lte("data", limiteCirurgico).order("data", { ascending: true }),
      supabase.from("agendamentos").select("data_cirurgia").in("status", ["confirmado", "realizado"]).not("data_cirurgia", "is", null),
    ]);
    ...
    datasCirurgiaDisponiveis = (datasCirurgia ?? []).map((d: any) => ({ id: d.id, data: d.data, vagasRestantes: Math.max(0, 1 - (ocupacaoCirurgia.get(d.data) ?? 0)) }));
  }
  ...
  return json({
    ...
    agendaCirurgicaLiberarEm: limiteCirurgico,
    agendaCirurgicaLiberada: elegivelCirurgia,
  });
}

export async function agendar(request: Request, env: Env): Promise<Response> {
  const s = await sessao(request, env); if (!s) return json({ erro: "Sessão expirada." }, 401);
  let body: any; try { body = await request.json(); } catch { return json({ erro: "Requisição inválida." }, 400); }
  const dataId = body?.dataId as string | undefined; const horario = body?.horario as string | undefined;
  ...
  if (cliente.status_revisao_financeira !== "aprovada" || cliente.financeiro_saldo_restante == null) return json({ erro: "O levantamento financeiro ainda não foi concluído." }, 409);
  const { data: custeioAprovado } = await supabase.from("solicitacoes_liberacao_financeira").select("id").eq("cliente_id", cliente.id).eq("status", "aprovada").order("created_at", { ascending: false }).limit(1).maybeSingle();
  ...
  if (error) {
    ...
    requestLogger(request).error("Falha ao confirmar agendamento de termos", { action: "client.terms.schedule", actorType: "cliente", actorId: await pseudonymizeActorId(cliente.id, env), eventCode: "TERMS_SCHEDULE_FAILED", error });
...
export async function agendarCirurgia(request: Request, env: Env): Promise<Response> {
  ...
  const hoje = agoraSaoPaulo().data;
  if (data < hoje) return json({ erro: "A data da cirurgia não pode estar no passado." }, 409);

  const supabase = createServiceSupabaseClient(env);
  const { data: cliente } = await supabase.from("clientes").select("id,custeio_confirmado_em").eq("id", s.clienteId).single();
  if (!cliente) return json({ erro: "Cliente não encontrada." }, 404);
  const { data: agendamento } = await supabase.from("agendamentos")
    .select("id,data_id,status,termos_assinados_em,data_cirurgia,datas(data)").eq("cliente_id", cliente.id).in("status", ["realizado", "confirmado"]).order("created_at", { ascending: false }).limit(1).maybeSingle();
  ...
  const limite = calcularLiberacaoCirurgica(agendamento.termos_assinados_em, cliente.custeio_confirmado_em);
  if (!limite) return json({ erro: "Não foi possível calcular o prazo da agenda cirúrgica." }, 409);
  if (data > limite) return json({ erro: `Essa data excede o prazo máximo (até ${limite.split("-").reverse().join("/")}).`, agendaCirurgicaLiberarEm: limite }, 409);

  const { data: solicitacao } = await supabase.from("solicitacoes_liberacao_financeira").select("id").eq("cliente_id", cliente.id).order("created_at", { ascending: false }).limit(1).maybeSingle();
  ...
  if (error) {
    ...
    if (m.includes("PRAZO_CIRURGICO_EXCEDIDO") || m.includes("DATA_CIRURGIA_ANTES_DA_BASE")) return json({ erro: "Essa data está fora da janela permitida para sua cirurgia." }, 409);
    ...
    requestLogger(request).error("Falha ao confirmar data da cirurgia", { action: "client.surgery.schedule", actorType: "cliente", actorId: await pseudonymizeActorId(cliente.id, env), eventCode: "SURGERY_SCHEDULE_FAILED", error });
  ...
  return json({ ok: true, data, agendaCirurgicaLiberarEm: limite });
}
...
export async function solicitarLiberacaoFinanceira(request: Request, env: Env): Promise<Response> {
  ...
  const { data: cliente } = await supabase.from("clientes")
    .select("id,status_revisao_financeira,financeiro_saldo_restante,financeiro_taxa_cartao,financeiro_total_com_taxa,financeiro_formas_custeio")
    .eq("id", s.clienteId).single();
  if (!cliente) return json({ erro: "Cliente não encontrada." }, 404);
  if (cliente.status_revisao_financeira !== "aprovada" || cliente.financeiro_saldo_restante == null) return json({ erro: "O levantamento financeiro ainda não foi confirmado pela nossa equipe." }, 409);
  ...
  const totalComTaxa = formaCusteio === "cartao" ? Math.round((saldoRestante * (1 + taxaCartao / 100)) * 100) / 100 : saldoRestante;
  const { data: agendamentoAtivo } = await supabase.from("agendamentos").select("id").eq("cliente_id", cliente.id).eq("status", "confirmado").order("created_at", { ascending: false }).limit(1).maybeSingle();
  const { data: existente } = await supabase.from("solicitacoes_liberacao_financeira").select("id,status,agendamento_id").eq("cliente_id", cliente.id).in("status", ["pendente", "em_analise", "aprovada"]).order("created_at", { ascending: false }).limit(1).maybeSingle();
  if (existente) {
    if (!existente.agendamento_id && agendamentoAtivo?.id) {
      const { data: vinculada, error } = await supabase.from("solicitacoes_liberacao_financeira").update({ agendamento_id: agendamentoAtivo.id }).eq("id", existente.id).select("id,forma_custeio,saldo_restante,taxa_cartao,total_com_taxa,status,agendamento_id").single();
      if (error) return json({ erro: "Não foi possível atualizar sua solicitação." }, 500);
      return json({ solicitacao: vinculada });
    }
    return json({ erro: "Sua escolha de custeio já foi enviada para nossa equipe." }, 409);
  }
  const observacao = formaCusteio === "cheques" || formaCusteio === "boleto_100" ? "Forma de custeio sujeita a análise de até 5 dias úteis." : "A escolha foi registrada e retornou para o painel administrativo.";
  const { data, error } = await supabase.from("solicitacoes_liberacao_financeira").insert({ cliente_id: cliente.id, agendamento_id: agendamentoAtivo?.id ?? null, forma_custeio: formaCusteio, saldo_restante: saldoRestante, taxa_cartao: taxaCartao, total_com_taxa: totalComTaxa, status: "pendente", observacao }).select("id,forma_custeio,saldo_restante,taxa_cartao,total_com_taxa,status,observacao,agendamento_id,created_at").single();
  if (error) return json({ erro: "Não foi possível registrar sua escolha de custeio." }, 500);
  await supabase.from("logs_alteracoes").insert({ usuario: `cliente:${cliente.id}`, acao: "solicitou_liberacao_financeira", entidade: "solicitacoes_liberacao_financeira", entidade_id: data.id, detalhes: { cliente_id: cliente.id, formaCusteio, saldoRestante, taxaCartao, totalComTaxa, agendamentoId: agendamentoAtivo?.id ?? null } });
...
export async function remarcarAgendamento(request: Request, env: Env): Promise<Response> {
  ...
  if (tipo === "termos" && (!dataId || !horario || !HORARIOS_VALIDOS.has(horario))) return json({ erro: "Escolha a nova data e o horário da assinatura." }, 400);
  if (tipo === "cirurgia" && (!dataEscolhida || !dataValida(dataEscolhida))) return json({ erro: "Escolha a nova data da cirurgia." }, 400);

  const supabase = createServiceSupabaseClient(env);
  const { data: agendamento } = await supabase.from("agendamentos")
    .select("id,data_id,status,horario_termos,termos_assinados_em,data_cirurgia,datas(data)")
    .eq("cliente_id", s.clienteId).in("status", ["confirmado", "realizado"]).order("created_at", { ascending: false }).limit(1).maybeSingle();
  if (!agendamento) return json({ erro: "Não existe um agendamento para alterar." }, 409);

  if (tipo === "termos") {
    if (agendamento.termos_assinados_em) return json({ erro: "Os termos já foram assinados e essa data não pode mais ser alterada pela área da cliente." }, 409);
    const { data: jaUsada } = await supabase.from("solicitacoes_remarcacao_agendamento").select("id").eq("agendamento_id", agendamento.id).eq("tipo", "termos").maybeSingle();
    if (jaUsada) return json({ erro: "Você já utilizou sua única alteração de data para a assinatura dos termos." }, 409);
  }
  ...
  if (tipo === "termos") {
    ...
    const hoje = agoraSaoPaulo().data;
    if (dataEscolhida! < hoje) return json({ erro: "A nova data não pode estar no passado." }, 409);
    ...
    if (!limite) return json({ erro: "A janela da agenda cirúrgica ainda não está disponível." }, 409);
    if (dataEscolhida! > limite) return json({ erro: `A data deve respeitar o prazo máximo até ${limite.split("-").reverse().join("/")}.`, agendaCirurgicaLiberarEm: limite }, 409);
  ...
  const payload = { cliente_id: s.clienteId, agendamento_id: agendamento.id, tipo, data_id: tipo === "termos" ? dataId! : null, data_solicitada: dataSolicitada, horario_termos: tipo === "termos" ? horario! : null, status: "pendente", updated_at: new Date().toISOString() };
  ...
  if (error) {
    requestLogger(request).error("Falha ao solicitar remarcação", { action: "client.schedule.change.request", actorType: "cliente", actorId: await pseudonymizeActorId(s.clienteId, env), eventCode: "RESCHEDULE_REQUEST_FAILED", error });
    return json({ erro: "Não foi possível enviar sua solicitação de alteração." }, 500);
  }
  return json({ ok: true, solicitacao, mensagem: "Sua solicitação de alteração foi enviada para análise. Sua agenda atual permanece inalterada até a autorização." });
}
