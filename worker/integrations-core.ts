...
async function handleContaAzulAdmin(request: Request, env: Env) {
  ...
  if (path === "/api/admin/integrations/conta-azul/create-receivable" && request.method === "POST") {
    ...
    try {
      ...
      await db.from("integracao_eventos").insert({ provedor: "conta_azul", event_id: String(result?.id || crypto.randomUUID()), event_type: "conta_receber_criada", referencia: boleto.id, payload: result, status: "processado", processado_em: new Date().toISOString() });
      return json({ ok: true, data: result });
    } catch (error) {
      console.error("Erro Conta Azul:", error);
      return json({ erro: "Não foi possível criar a conta a receber no Conta Azul." }, 502);
    }
  }

  if (path === "/api/admin/integrations/conta-azul/list-receivables" && request.method === "GET") {
    try {
      const data = await contaAzulRequest(env, `/v1/financeiro/eventos-financeiros/contas-a-receber?pagina=1&tamanho_pagina=50`);
      return json({ data });
    } catch (error) {
      console.error("Erro Conta Azul:", error);
      return json({ erro: "Não foi possível consultar o Conta Azul." }, 502);
    }
  }
  return null;
}

async function auditClientSession(request: Request, env: Env) {
  const client = await requireClient(request, env);
  if (!client) return json({ erro: "Sessão expirada." }, 401);
  const db = createServiceSupabaseClient(env);
  const { data: cliente, error } = await db.from("clientes").select("id,ativo,status_contrato").eq("id", client).maybeSingle();
  if (error) return json({ erro: "Não foi possível validar a sessão." }, 503);
  if (!cliente || cliente.ativo !== true || ["suspenso", "negativado", "cancelado"].includes(String(cliente.status_contrato || "").toLowerCase())) {
...
export async function integrationsApi(request: Request, env: Env): Promise<Response | null> {
  ...
  const credenciais = await credenciaisApi(request, env);
  ...
  if (path === "/api/cliente/payments/mercado-pago/preference" && request.method === "POST") {
    const sessionDenied = await auditClientSession(request, env);
