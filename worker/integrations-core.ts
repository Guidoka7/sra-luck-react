...
async function createMercadoPagoPreference(request: Request, env: Env) {
  ...
  if (!response.ok || !mp?.id || !mp?.init_point) {
    log.error("Falha ao criar preferência Mercado Pago", { entityType: "boleto", entityId: boleto.id, eventCode: "MP_PREFERENCE_FAILED", statusCode: response.status || 502, providerStatus: response.status });
...
async function handleMercadoPagoWebhook(request: Request, env: Env) {
  ...
  if (!response.ok) {
    log.error("Falha ao consultar pagamento Mercado Pago", { eventCode: "MP_PAYMENT_LOOKUP_FAILED", statusCode: response.status || 502, providerStatus: response.status, externalPaymentId: paymentId });
  ...
  if (statusProvedor === "approved" && boleto.status !== "pago") {
    ...
    const { error: pendingError } = await db.from("boletos").update({ status: "pendente_confirmacao", observacoes }).eq("id", boleto.id).neq("status", "pago");
    if (pendingError) log.error("Pagamento aprovado no provedor, mas parcela não entrou na fila de conferência", { entityType: "boleto", entityId: boleto.id, eventCode: "MP_PENDING_CONFIRMATION_FAILED", error: pendingError });
  }
  ...
  const { error: eventError } = await db.from("integracao_eventos").upsert({
    ...
    payload: payment, status: "processado", processado_em: new Date().toISOString(),
  }, { onConflict: "provedor,event_id", ignoreDuplicates: false });
  if (eventError) log.warn("Webhook processado, mas evento de integração não foi persistido", { entityType: "boleto", entityId: boleto.id, eventCode: "MP_INTEGRATION_EVENT_FAILED", error: eventError });

  const { error: auditError } = await db.from("logs_alteracoes").insert({
    usuario: "sistema:mercado_pago", acao: "recebeu_evento_mercado_pago", entidade: "boletos", entidade_id: boleto.id,
    detalhes: { paymentId, statusProvedor, transactionAmount: payment.transaction_amount ?? null, cliente_id: boleto.cliente_id, baixaAutomatica: false, aguardandoConferenciaHumana: statusProvedor === "approved" },
  });
...
async function handleContaAzulAdmin(request: Request, env: Env) {
  ...
  if (path === "/api/admin/integrations/conta-azul/create-receivable" && request.method === "POST") {
    ...
    try {
      const result = await contaAzulRequest(env, "/v1/financeiro/eventos-financeiros/contas-a-receber", { method: "POST", body: JSON.stringify(payload) }) as any;
      await db.from("integracao_eventos").insert({ provedor: "conta_azul", event_id: String(result?.id || crypto.randomUUID()), event_type: "conta_receber_criada", referencia: boleto.id, payload: { boleto_id: boleto.id }, status: "processado", processado_em: new Date().toISOString() });
      return json({ ok: true, id: result?.id || null });
    } catch (error) {
      requestLogger(request).child({ actorType: "admin", actorId: await pseudonymizeActorId(admin, env), action: "conta_azul.receivable.create", provider: "conta_azul" }).error("Falha ao criar conta a receber no Conta Azul", { entityType: "boleto", entityId: boleto.id, eventCode: "CONTA_AZUL_RECEIVABLE_FAILED", statusCode: 502, error });
      return json({ erro: "Falha ao criar conta a receber no Conta Azul." }, 502);
    }
  }
...
export async function integrationsApi(request: Request, env: Env): Promise<Response | null> {
  ...
  const creds = await credenciaisApi(request, env);
  if (creds) return creds;
  const rdReadonly = await rdStationReadonlyApi(request, env);
  if (rdReadonly) return rdReadonly;
  const pushConfig = await webPushConfigApi(request, env);
