...
// Dados financeiros e pessoais exigem sessões curtas e revalidadas.
const MAX_AGE_SECONDS = 60 * 60 * 24;
const ADMIN_MAX_AGE_SECONDS = 60 * 60 * 8;
const STAFF_MAX_AGE_SECONDS = 60 * 60 * 8;
...
export interface StaffSessionPayload {
  staffId: string;
  authUserId: string;
  role: "vendedora" | "sdr" | "financeiro" | "gestao" | "administrativo";
  iat: number;
}
...
function cookie(name: string, token: string, maxAge: number, path: string, secure: boolean) {
  return `${name}=${token}; Max-Age=${maxAge}; Path=${path}; HttpOnly; SameSite=Lax; Priority=High${secure ? "; Secure" : ""}`;
}

export function setSessionCookie(token: string, secure: boolean): string { return cookie(COOKIE_NAME, token, MAX_AGE_SECONDS, "/", secure); }
export function clearSessionCookie(secure: boolean): string { return cookie(COOKIE_NAME, "", 0, "/", secure); }
export function setAdminSessionCookie(token: string, secure: boolean): string { return cookie(ADMIN_COOKIE_NAME, token, ADMIN_MAX_AGE_SECONDS, "/api/admin", secure); }
export function clearAdminSessionCookie(secure: boolean): string { return cookie(ADMIN_COOKIE_NAME, "", 0, "/api/admin", secure); }
export function setStaffSessionCookie(token: string, secure: boolean): string { return cookie(STAFF_COOKIE_NAME, token, STAFF_MAX_AGE_SECONDS, "/api/equipe", secure); }
export function clearStaffSessionCookie(secure: boolean): string { return cookie(STAFF_COOKIE_NAME, "", 0, "/api/equipe", secure); }
