...
export async function adminApi(request: Request, env: Env): Promise<Response | null> {
  ...
  const auth=await exigirAdmin(request,env);if(auth)return auth;const supabase=createServiceSupabaseClient(env,request);
  const clientesPaginados = await adminClientPage(request, env);
  if (clientesPaginados) return clientesPaginados;
