...
export async function clientPushApi(request: Request, env: Env): Promise<Response | null> {
  ...
  const log = baseLog.child({ actorType: "cliente", actorId: await pseudonymizeActorId(client, env), action: "push.subscription" });
  ...
  if (path === "/api/cliente/push/subscribe" && request.method === "POST") {
    ...
    if (erroConsulta) {
      log.error("Falha ao validar assinatura push existente", { eventCode: "PUSH_SUBSCRIPTION_LOOKUP_FAILED", statusCode: 500, error: erroConsulta });
      return json({ erro: "Não foi possível validar este dispositivo." }, 500);
    }
    if (existente && existente.cliente_id !== client) return json({ erro: "Este dispositivo já possui uma assinatura vinculada." }, 409);
    ...
    if (error) {
      log.error("Falha ao registrar assinatura push", { eventCode: "PUSH_SUBSCRIPTION_SAVE_FAILED", statusCode: 500, error });
  ...
  if (path === "/api/cliente/push/unsubscribe" && (request.method === "POST" || request.method === "DELETE")) {
    ...
    if (error) {
      log.error("Falha ao remover assinatura push", { eventCode: "PUSH_SUBSCRIPTION_DELETE_FAILED", statusCode: 500, error });
