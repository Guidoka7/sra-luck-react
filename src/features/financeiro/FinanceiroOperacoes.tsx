...
function Dialog({ title, eyebrow, children, onClose, wide = false }: { title: string; eyebrow: string; children: ReactNode; onClose: () => void; wide?: boolean }) {
  return <div className="fixed inset-0 z-[100] isolate flex items-center justify-center bg-black/55 p-3 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section role="dialog" aria-modal="true" aria-label={title} className={`relative z-10 max-h-[94vh] w-full overflow-y-auto rounded-2xl border border-rose/15 bg-white p-5 shadow-2xl dark:border-white/10 dark:bg-[#17181D] ${wide ? "max-w-3xl" : "max-w-lg"}`}>
...
export function BaixaManualModal({ recebivel, onClose, onSuccess }: { recebivel: Recebivel; onClose: () => void; onSuccess: () => void }) {
  ...
  const [juros, setJuros] = useState("0"); const [multa, setMulta] = useState("0");
  ...
  const total = useMemo(() => Math.max(0, recebivel.valorOriginal + Number(juros || 0) + Number(multa || 0)), [recebivel.valorOriginal, juros, multa]);
  ...
  async function salvar(event: FormEvent) {
    ...
    try {
      ...
      await financeiroApi.baixa(recebivel.id, { dataPagamento, juros: Number(juros), multa: Number(multa), formaPagamento, instituicaoConta, observacao, idempotencyKey });
      onSuccess();
  ...
  return <Dialog eyebrow="Liquidação auditável" title={`Baixa manual · parcela ${recebivel.numeroParcela}/${recebivel.totalParcelas}`} onClose={onClose} wide>
    <form onSubmit={salvar} className="mt-5 space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Field label="Data do pagamento"><Input required type="date" value={dataPagamento} onChange={(e) => setDataPagamento(e.target.value)} /></Field><Field label="Valor original"><Input disabled value={recebivel.valorOriginal.toFixed(2)} /></Field><Field label="Juros"><Input min="0" step="0.01" type="number" value={juros} onChange={(e) => setJuros(e.target.value)} /></Field><Field label="Multa"><Input min="0" step="0.01" type="number" value={multa} onChange={(e) => setMulta(e.target.value)} /></Field><Field label="Recebido final"><Input disabled value={total.toFixed(2)} /></Field><Field label="Forma de pagamento"><Select value={formaPagamento} onChange={(e) => setFormaPagamento(e.target.value)}><option value="pix">PIX</option><option value="dinheiro">Dinheiro</option><option value="transferencia">Transferência</option><option value="boleto">Boleto</option><option value="cartao">Cartão via terminal/provedor</option><option value="cheque">Cheque</option><option value="outro">Outro</option></Select></Field><Field label="Banco"><Input value={instituicaoConta} onChange={(e) => setInstituicaoConta(e.target.value)} placeholder="Ex.: Itaú, Nubank, Caixa" /></Field></div>
      ...
      {erro ? <p role="alert" className="rounded-xl bg-alert/10 p-3 text-xs text-alert dark:bg-[#C7869B]/10 dark:text-[#DCA0B2]">{erro}</p> : null}
...
export function RecebivelDrawer({ id, onClose, onAction, onUpdated }: { id: string; onClose: () => void; onAction: (action: "baixa" | "editar" | "validar", item: Recebivel) => void; onUpdated: () => void }) {
  ...
  return <Portal><div className="fixed inset-0 z-[110] bg-[#241015]/55 backdrop-blur-[3px]" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <aside role="dialog" aria-modal="true" aria-labelledby="recebivel-drawer-title" className="fixed inset-y-0 right-0 flex w-full max-w-[460px] flex-col overflow-hidden border-l border-[#ead9da] bg-[#fffaf8] text-clay shadow-[-24px_0_70px_-30px_rgba(31,12,17,.55)] dark:border-white/10 dark:bg-[#171316] dark:text-[#e7dedd]">
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain bg-[#f8f3f1] px-4 py-4 dark:bg-[#101115]">
        ...
        {erro ? <p role="alert" className="rounded-xl border border-alert/15 bg-alert/[0.07] p-3 text-xs text-alert dark:border-[#C7869B]/30 dark:bg-[#C7869B]/10 dark:text-[#DCA0B2]">{erro}</p> : null}
        {recebivel ? <div className="space-y-3">
          <section className="overflow-hidden rounded-2xl border border-rose/10 bg-white shadow-[0_12px_35px_-30px_rgba(72,24,35,.45)] dark:border-white/8 dark:bg-[#17181D]">
          ...
          <section className="rounded-2xl border border-rose/10 bg-white p-4 dark:border-white/8 dark:bg-[#17181D]">
