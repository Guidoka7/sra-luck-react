...
type Logo = { fundo: string; cor: string; conteudo: ReactNode; borda?: boolean };

function simbolo(path: string, fill: string) {
  return <svg viewBox="0 0 24 24" width="60%" height="60%" aria-hidden><path d={path} fill={fill} /></svg>;
}

function monograma(texto: string, tamanho = 12, estilo: "normal" | "italic" = "normal") {
  return <span style={{ fontSize: tamanho, fontWeight: 800, letterSpacing: "-0.02em", fontStyle: estilo, lineHeight: 1 }}>{texto}</span>;
}

const LOGOS: Record<string, Logo> = {
  web_push: {
    fundo: "linear-gradient(145deg,#B0526A,#7A2632)", cor: "#FFFFFF",
    conteudo: <svg viewBox="0 0 24 24" width="52%" height="52%" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9" /><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0" /></svg>,
  },
  mercado_pago: { fundo: "#00B1EA", cor: "#FFFFFF", conteudo: simbolo(PATH_MERCADO_PAGO, "#FFFFFF") },
  gemini: {
    fundo: "#FFFFFF", cor: "#8E75B2", borda: true,
    conteudo: <svg viewBox="0 0 24 24" width="62%" height="62%" aria-hidden><defs><linearGradient id="sl-gemini" x1="0" y1="1" x2="1" y2="0"><stop offset="0" stopColor="#1C7DFF" /><stop offset=".55" stopColor="#8E75B2" /><stop offset="1" stopColor="#E86C7F" /></linearGradient></defs><path d={PATH_GEMINI} fill="url(#sl-gemini)" /></svg>,
  },
  conta_azul: { fundo: "#1E7BE0", cor: "#FFFFFF", conteudo: monograma("ca", 15) },
  rd_station: { fundo: "#0B2239", cor: "#FFFFFF", conteudo: monograma("RD", 13) },
  brb: { fundo: "#0033A1", cor: "#FFFFFF", conteudo: monograma("BRB", 10.5) },
  bb: { fundo: "#FCEF1B", cor: "#0038A8", conteudo: monograma("BB", 13) },
  santander: { fundo: "#EC0000", cor: "#FFFFFF", conteudo: monograma("S", 16) },
  sicredi: { fundo: "#3FA110", cor: "#FFFFFF", conteudo: monograma("S", 16) },
  efi: { fundo: "#F37021", cor: "#FFFFFF", conteudo: monograma("efí", 13, "italic") },
};

export function LogoIntegracao({ id, nome, tamanho = 40 }: { id: string; nome: string; tamanho?: number }) {
  const logo = LOGOS[id] ?? { fundo: "var(--robg)", cor: "var(--bg)", conteudo: monograma(nome.slice(0, 2).toUpperCase(), 12) };
  return (
    <div
      role="img"
      aria-label={nome}
      style={{
        ...
        borderRadius: Math.round(tamanho * 0.26), background: logo.fundo, color: logo.cor,
        border: logo.borda ? "1px solid var(--line)" : "none",
        boxShadow: "0 1px 2px rgba(20,10,12,.08), inset 0 0 0 .5px rgba(255,255,255,.2)",
