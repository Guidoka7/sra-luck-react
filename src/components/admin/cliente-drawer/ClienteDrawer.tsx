...
function ExclusaoModal({ cad }: { cad: ClienteCadastro }) {
  return <Shell titulo="Excluir perfil da cliente" onClose={() => { if (!cad.excluindo) cad.setConfirmarExclusao(false); }}>
    <div className={styles.warning}>O perfil de <b>{cad.nome || "esta cliente"}</b> será removido das áreas operacionais. Se houver histórico financeiro ou de agenda, ele será preservado para auditoria; sem histórico, o cadastro poderá ser excluído definitivamente. Em ambos os casos, o CPF fica livre para um novo cadastro e a venda do RD volta para "aguardando cadastro" (nada é alterado no RD Station).</div>
