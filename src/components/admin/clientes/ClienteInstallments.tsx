  ...
  return <>
    ...
    {modal ? <ModalShell title={modalTitle(modal)} onClose={() => !busy && setModal(null)}>
      ...
      {modal.type === "edit" ? <form onSubmit={(e)=>void saveEdit(e,modal.item)}><div className={styles.modalGrid}>
        ...
        <Field label="Vencimento"><input className={styles.input} name="dueDate" type="date" required defaultValue={modal.item.dueDate}/></Field>
        <Field label="Valor"><input className={styles.input} name="value" type="number" min="0.01" step="0.01" required defaultValue={modal.item.value}/></Field>
...
function ModalShell({title,onClose,children}:{title:string;onClose:()=>void;children:React.ReactNode}) { return <div className={styles.modalLayer} role="dialog" aria-modal="true" aria-label={title} onMouseDown={(e)=>{if(e.target===e.currentTarget)onClose();}}><div className={styles.modal}><div className={styles.modalHead}><h4>{title}</h4><button className={styles.iconBtn} type="button" onClick={onClose} aria-label="Fechar"><DrawerIcon name="close"/></button></div><div className={styles.modalBody}>{children}</div></div></div>; }
...
function RejectForm({busy,item,onCancel,onSubmit}:{busy:boolean;item:DrawerInstallment;onCancel:()=>void;onSubmit:(reason:string)=>void}) { const [reason,setReason]=useState(""); return <form onSubmit={(e)=>{e.preventDefault();if(reason.trim())onSubmit(reason.trim());}}><div className={styles.warning}>Informe o motivo da rejeição do comprovante da parcela {item.number}.</div><div className={styles.field} style={{marginTop:10}}><label>Motivo</label><textarea className={styles.input} value={reason} onChange={(e)=>setReason(e.target.value)} required/></div><Actions busy={busy} submitLabel="Rejeitar comprovante" onCancel={onCancel}/></form>; }
