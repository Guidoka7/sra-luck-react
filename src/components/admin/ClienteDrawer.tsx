...
export function ClienteDrawer({ cliente, onClose, onSalvo, abaInicial = "perfil" }: { cliente: Cliente | null; onClose: () => void; onSalvo: () => void; abaInicial?: "perfil" | "financeiro" }) {
  ...
  return <>
    ...
    <aside className="fixed inset-y-0 right-0 z-50 flex w-full max-w-[420px] flex-col border-l border-slate-200/80 bg-white text-slate-900 shadow-[-24px_0_60px_-30px_rgba(0,0,0,.35)] animate-slideInRight dark:border-white/10 dark:bg-[#18161a] dark:text-slate-100">
      <header className="flex shrink-0 items-center justify-between gap-2 border-b border-slate-200 px-4 py-3.5 dark:border-white/10">
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <div className="min-w-0">
            ...
            {editando && <span className="mt-0.5 inline-flex items-center gap-1 text-[0.6rem] font-semibold uppercase tracking-label text-slate-500 dark:text-slate-400"><span className={`h-1.5 w-1.5 rounded-full ${statusContrato === "ativo" ? "bg-success" : statusContrato === "suspenso" ? "bg-amber-500" : "bg-alert"}`} />{STATUS_CONTRATO_LABEL[cliente?.status_contrato ?? "ativo"]}</span>}
          </div>
        </div>
        <div className="flex items-center gap-1.5">
          {editando && <button type="button" onClick={() => setConfirmarExclusao(true)} className="inline-flex items-center gap-1.5 rounded-lg border border-alert/30 bg-alert/10 px-2.5 py-2 text-[0.58rem] font-bold text-alert hover:bg-alert/20"><Trash2 className="h-3.5 w-3.5" /></button>}
      ...
      <div className="border-b border-slate-200 bg-slate-50 px-3 py-2 dark:border-white/8 dark:bg-[#141216]">
