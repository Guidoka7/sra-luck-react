...
function ClockIcon() {
  return <svg width="14" height="14" viewBox="0 0 18 18" fill="none" stroke="currentColor" strokeWidth="1.25"><circle cx="9" cy="9" r="6.2"/><path d="M9 5.6V9l2.3 1.5"/></svg>;
}
...
export function AgendaEtapasInterativas({ atual, percentual, parcelasNecessarias, onPagamentoClick }: Props) {
  ...
  return (
    <div>
      <div className="rounded-[14px] border border-[#E9DEDA] bg-white/95 px-[12px] py-[11px] shadow-[0_5px_16px_rgba(73,42,45,.045)]">
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-center gap-[9px]">
            <span className={`flex h-[31px] w-[31px] flex-none items-center justify-center rounded-[10px] ${selecionada === "levantamento" || selecionada === "pagamento" ? "bg-[#F8EEDB] text-[#A77A24]" : selecionada === "data" ? "bg-[#EAF4EC] text-[#3F7D5B]" : "bg-[#F7E9EA] text-[#B65B67]"}`}>
              {icon(selecionada)}
            </span>
            <div className="min-w-0">
              <div className="text-[7.5px] font-bold uppercase tracking-[.14em] text-[#A99894]">Etapa {indiceSelecionado + 1} de 4</div>
              <div className="pt-[2px] font-heading text-[15.5px] font-semibold leading-tight text-[#7D2434]">{tituloCompleto(selecionada)}</div>
            </div>
          </div>
          <span className={`flex-none rounded-full border px-[7px] py-[4px] text-[7.5px] font-semibold uppercase tracking-[.05em] ${concluida ? "border-[#D5E8D9] bg-[#F0F7F1] text-[#3F7D5B]" : atualSelecionada ? "border-[#E8D2A9] bg-[#FFF9EF] text-[#8E6420]" : "border-[#E7DEDB] bg-[#F8F4F3] text-[#8A7B77]"}`}>{status}</span>
        </div>
        <p className="pt-[8px] text-[9.8px] font-light leading-[1.48] text-[#786A66]">{texto}</p>
        ...
        {selecionada === "pagamento" && atual === "pagamento" && onPagamentoClick && (
          <motion.button
            ...
            className="mt-[9px] flex w-full items-center justify-center gap-[7px] rounded-[10px] border border-[#E1C48C] bg-[#FFF7E8] px-3 py-[9px] text-[9.5px] font-semibold text-[#7D5B1E] shadow-[0_4px_12px_rgba(167,122,36,.08)]"
          >
            <span className="relative flex h-[7px] w-[7px] items-center justify-center">
              <span className="absolute h-[7px] w-[7px] animate-ping rounded-full bg-[#B7862A]/35" />
              <span className="relative h-[5px] w-[5px] rounded-full bg-[#B7862A]" />
            </span>
            Toque aqui para escolher a forma de pagamento
          </motion.button>
        )}
      </div>

      <div className="relative mt-[10px] grid grid-cols-4 gap-[5px]">
        <div className="pointer-events-none absolute left-[12.5%] right-[12.5%] top-[17px] h-px bg-[#E6D9D5]" />
        <div className="pointer-events-none absolute left-[12.5%] top-[17px] h-px bg-[#C89D45]" style={{ width: `${Math.max(0, indiceAtual) * 25}%` }} />
        {ETAPAS.map((item, index) => {
          ...
          const botao = (
            <button
              ...
              aria-pressed={ativa}
              className={`relative z-[1] min-w-0 rounded-[11px] border px-[3px] pb-[7px] pt-[5px] text-center transition ${ativa ? "border-[#D8BD86] bg-[#FFF9EF] shadow-[0_4px_12px_rgba(127,91,30,.07)]" : "border-[#EDE4E1] bg-white/95"}`}
            >
              {pagamentoChamando && <span className="absolute -right-[3px] -top-[5px] rounded-full bg-[#7D2434] px-[5px] py-[2px] text-[5.8px] font-bold uppercase tracking-[.06em] text-white">Toque</span>}
              <span className={`mx-auto flex h-[24px] w-[24px] items-center justify-center rounded-full border ${feita ? "border-[#CFE2D3] bg-[#EAF4EC] text-[#3F7D5B]" : corrente ? "border-[#E4C98E] bg-[#FBF1DD] text-[#A77A24]" : "border-[#E5DBD8] bg-[#F5F1EF] text-[#9A8A86]"}`}>
              ...
              <span className={`block pt-[2px] text-[7.5px] font-semibold leading-[1.15] ${ativa ? "text-[#7D2434]" : "text-[#71615E]"}`}>{item.titulo}</span>
            </button>
          );

          if (!pagamentoChamando) return <div key={item.id}>{botao}</div>;
          return (
            <motion.div key={item.id} animate={{ y: [0, -2, 0] }} transition={{ duration: 1.45, repeat: Infinity, ease: "easeInOut" }}>
              {botao}
            </motion.div>
          );
        })}
