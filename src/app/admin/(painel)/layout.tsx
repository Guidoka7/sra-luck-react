...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="admin-web-shell admin-shell admin-compact min-h-screen bg-bloom dark:bg-[#0b0a0c]">
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-[1840px] gap-4 px-4 py-4 xl:gap-5 xl:px-5">
        <aside className="admin-sidebar flex flex-col rounded-2xl border border-white/60 bg-white/88 px-3.5 py-3.5 text-clay shadow-[0_30px_100px_-40px_rgba(122,38,50,0.28)] backdrop-blur-2xl dark:border-white/8 dark:bg-[#151317]/96 dark:text-[#e8dcda] dark:shadow-[0_30px_90px_-28px_rgba(0,0,0,0.72)]">
          <div>
            <div className="mb-5 min-w-0 px-1">
              <Wordmark maxWidth={170} />
            ...
            <nav className="space-y-5" aria-label="Navegação administrativa">
              {Array.from(new Set(NAV.map((item) => item.group))).map((group) => (
                <div key={group}>
                  <p className="mb-1.5 px-2 text-[9px] font-semibold uppercase tracking-[0.24em] text-burgundy/38 dark:text-white/35">
                    {group}
                  </p>
                    ...
                    {NAV.filter((item) => item.group === group).map((item) => {
                      ...
                      return (
                        <Link
                          ...
                          className={cn(
                            "group flex items-center gap-3 rounded-xl px-2.5 py-2.5 text-[13px] transition-all duration-200",
                            ativo
                              ? "bg-burgundy text-pearl shadow-[0_10px_24px_-12px_rgba(122,38,50,0.72)] dark:bg-[#7f3546] dark:text-[#fff7f4] dark:shadow-[0_10px_26px_-12px_rgba(0,0,0,0.8)]"
                        ...
                        >
                          <span
                            className={cn(
                              ...
                              ativo
                                ? "bg-white/16 text-pearl ring-1 ring-white/10 dark:bg-white/10 dark:text-[#fff8f5]"
                                : "bg-blush/60 text-burgundy group-hover:bg-blush dark:bg-white/6 dark:text-[#d9a5a3] dark:group-hover:bg-white/10",
                            )}
                          >
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p
                              className={cn(
                                "truncate text-[9px]",
                                ativo ? "text-pearl/72 dark:text-white/62" : "text-burgundy/40 dark:text-white/32",
                              )}
                            >
                              {item.group}
                            </p>
        ...
        <main className="admin-main px-0 py-0.5">{children}</main>
      </div>
    </div>
  );
}
