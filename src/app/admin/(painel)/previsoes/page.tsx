...
function MiniDelta({ value, helper }: { value: number | null; helper: string }) {
  ...
  return <span className={`inline-flex items-center gap-1 text-[9px] font-semibold ${positive ? "text-emerald-600" : "text-red-600"}`}><Icon className="h-3 w-3" />{Math.abs(value).toFixed(0)}% <span className="font-normal text-clay/42">{helper}</span></span>;
}
...
function KpiCard({ icon: Icon, label, value, delta, helper, tone = "rose" }: { icon: LucideIcon; label: string; value: string; delta?: number | null; helper: string; tone?: "rose" | "gold" | "alert" }) {
  ...
  return (
    <div className="rounded-xl border border-[#efdede] bg-gradient-to-br from-[#fff9f8] to-[#fcecec] px-3.5 py-3 shadow-[0_10px_30px_-26px_rgba(91,19,36,.35)] transition hover:-translate-y-px hover:shadow-[0_18px_35px_-28px_rgba(91,19,36,.4)]">
...
function DrawerShell({ children, onClose, level = 90, kind = "portfolio" }: { children: ReactNode; onClose: () => void; level?: number; kind?: "portfolio" | "profile" }) {
  return (
    <div className="fixed inset-0 flex items-stretch justify-end bg-[#1b1518]/52 backdrop-blur-[1.5px]" style={{ zIndex: level }} role="dialog" aria-modal="true">
      <button type="button" className="absolute inset-0 cursor-default" onClick={onClose} aria-label="Fechar painel lateral" />
      <aside className={`relative z-10 h-full overflow-y-auto border-l border-white/70 bg-[#fffdfc] shadow-[-34px_0_90px_-38px_rgba(55,14,26,.52)] ${kind === "profile" ? "w-full md:w-[94%] lg:w-[80%] xl:w-[66%] 2xl:w-[55%]" : "w-full md:w-[94%] lg:w-[80%] xl:w-[66%] 2xl:w-[54%]"}`}>
        {children}
      </aside>
    </div>
  );
...
export default function PrevisoesPage() {
  ...
  const salesMonthClients = useMemo(() => filteredClients.filter((client) => {
    const financeiroAtivo = Boolean(client.primeiroBoletoEm || client.primeiroVencimento);
    const referencia = client.dataVenda || client.primeiroBoletoEm || client.primeiroVencimento;
    return financeiroAtivo && referencia?.slice(0, 7) === selectedMonth;
  }), [filteredClients, selectedMonth]);
  const previousSalesMonthClients = useMemo(() => allClients.filter((client) => {
    const financeiroAtivo = Boolean(client.primeiroBoletoEm || client.primeiroVencimento);
    const referencia = client.dataVenda || client.primeiroBoletoEm || client.primeiroVencimento;
    return financeiroAtivo && referencia?.slice(0, 7) === shiftMonth(selectedMonth, -1);
  ...
  const clientForecastMonth = selectedClient?.previsao?.slice(0, 7) || selectedMonth;
    ...
    <div className="pb-8 text-[#3f3034]">
      ...
      <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_292px]">
        <div className="min-w-0 space-y-3">
          ...
          <div className="grid gap-3 2xl:grid-cols-[minmax(280px,.9fr)_minmax(440px,1.42fr)_minmax(240px,.72fr)]">
            ...
            <section className="rounded-xl border border-[#eadedf] bg-white p-3 shadow-sm">
              <div className="flex items-center justify-between gap-3"><div><h2 className="flex items-center gap-2 text-[12px] font-semibold text-burgundy"><BarChart3 className="h-4 w-4" />Previsão de Liberações por Mês</h2><div className="mt-1 flex items-center gap-3 text-[8px] text-clay/50"><span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-burgundy" />Quantidade de liberações</span><span className="inline-flex items-center gap-1"><span className="h-[2px] w-4 bg-[#e99c9d]" />Valor previsto (R$)</span></div></div><span className="rounded-md border border-[#eadedf] px-2 py-1 text-[8px] text-clay/55">Mês + próximos 6</span></div>

              <div className="relative mt-4 h-[235px] select-none">
                <div className="absolute bottom-8 left-8 right-11 top-1">
                  ...
                  <span className="absolute -left-7 -top-1 text-[7px] text-clay/40">{maxChartCount}</span>
                  <span className="absolute -bottom-1 -left-4 text-[7px] text-clay/40">0</span>
                  <span className="absolute -right-11 -top-1 text-[7px] text-clay/40">{compactMoney(maxChartValue)}</span>
                  <span className="absolute -bottom-1 -right-7 text-[7px] text-clay/40">R$ 0</span>

                  <div className="absolute inset-0 flex items-end gap-2">
                    {chartData.map((item, index) => {
                      const active = hoveredChart === index;
                      const height = Math.max(item.count ? 14 : 2, (item.count / maxChartCount) * 130);
                      return <button key={item.month} type="button" onMouseEnter={() => setHoveredChart(index)} onMouseLeave={() => setHoveredChart(null)} onFocus={() => setHoveredChart(index)} onBlur={() => setHoveredChart(null)} onClick={() => setSelectedMonth(item.month)} className="group relative flex h-full min-w-0 flex-1 flex-col items-center justify-end outline-none">
                        {active ? <div className="absolute left-1/2 top-1 z-20 w-[150px] -translate-x-1/2 rounded-lg border border-[#ead9da] bg-white px-2.5 py-2 text-left shadow-[0_14px_36px_-14px_rgba(73,20,34,.28)]"><p className="text-[8px] font-semibold uppercase tracking-[.1em] text-clay/45">{monthLong(item.month)}</p><p className="mt-1 text-[10px] font-semibold text-burgundy">{item.count} liberações previstas</p><p className="mt-0.5 text-[10px] font-semibold text-[#c46e75]">{item.value > 0 ? formatarMoeda(item.value) : "Valor aguardando CRM"}</p></div> : null}
                        <span className={`mb-1 text-[9px] font-bold transition ${active ? "text-burgundy" : "text-[#3f2f33]"}`}>{item.count}</span>
                  ...
                  {chartData.some((item) => item.value > 0) ? <svg className="pointer-events-none absolute inset-0 h-full w-full overflow-visible" viewBox="0 0 700 160" preserveAspectRatio="none" aria-hidden="true"><polyline points={linePoints} fill="none" stroke="#e89a9c" strokeWidth="3" strokeLinejoin="round" strokeLinecap="round" vectorEffect="non-scaling-stroke" />{chartPoints.map((point, index) => <circle key={chartData[index].month} cx={point.x} cy={point.y} r={hoveredChart === index ? 5 : 3.5} fill="#fffdfc" stroke="#e89a9c" strokeWidth={hoveredChart === index ? 3 : 2} vectorEffect="non-scaling-stroke" />)}</svg> : null}
                </div>
                <div className="absolute bottom-0 left-8 right-11 flex gap-2">{chartData.map((item) => <span key={item.month} className={`min-w-0 flex-1 text-center text-[8px] ${item.month === selectedMonth ? "font-bold text-burgundy" : "text-clay/50"}`}>{monthShort(item.month)}</span>)}</div>
              </div>

              <div className="mt-1 grid grid-cols-[1fr_auto_1fr] items-center rounded-lg bg-[#fff0ef] px-3 py-2"><div><p className="text-[8px] text-burgundy/60">Total previsto no período</p><p className="text-[12px] font-semibold text-burgundy">{projectedNext6} liberações</p></div><div className="h-8 w-px bg-rose/30" /><div className="text-right"><p className="text-[12px] font-semibold text-burgundy">{projectedValueNext6 > 0 ? formatarMoeda(projectedValueNext6) : "Valor aguardando CRM"}</p><p className="text-[8px] text-burgundy/55">em valor de cartas</p></div></div>
            </section>
            ...
            <section className="rounded-xl border border-[#eadedf] bg-white p-3 shadow-sm">
              ...
              <p className="mt-1 text-[8px] leading-4 text-clay/45">Carteiras do mês analisado, calculadas pelo vencimento da parcela-alvo.</p>
              <div className="mt-3 space-y-1.5">{rules.map((rule) => { const clients = releaseMonthClients.filter((client) => client.totalParcelas === rule.parcelas); const count = clients.length; const value = clients.reduce((sum, client) => sum + Number(client.valorCarta || 0), 0); return <button type="button" key={rule.parcelas} onClick={() => { setSelectedPortfolio(rule.parcelas); setPortfolioPage(1); setPortfolioSearch(""); }} className="group flex w-full items-center gap-2 rounded-lg border border-[#efe5e4] bg-[#fffdfc] px-2 py-1.5 text-left transition hover:-translate-y-px hover:border-burgundy/20 hover:bg-[#fff9f8] hover:shadow-sm"><span className={`flex h-7 min-w-[48px] items-center justify-center rounded-md text-[10px] font-bold text-burgundy ${PLAN_ACCENTS[rule.parcelas] ?? "bg-blush"}`}>{rule.parcelas}x</span><span className="min-w-0 flex-1"><span className="block text-[9px] font-medium text-clay/65">{count} cliente{count === 1 ? "" : "s"}</span><span className="block truncate text-[7px] text-clay/38">{value > 0 ? compactMoney(value) : "valor pendente"}</span></span><span className="text-[9px] font-semibold text-burgundy">{rule.parcelasNecessarias}ª</span><ArrowUpRight className="h-3 w-3 text-burgundy/30 transition group-hover:text-burgundy" /></button>; })}</div>
            </section>
          </div>
      ...
      {selectedPortfolio != null && selectedRule ? (
        <DrawerShell onClose={() => setSelectedPortfolio(null)} level={90} kind="portfolio">
          <div className="sticky top-0 z-20 border-b border-[#eee3e3] bg-[#fffdfc]/95 px-5 py-4 backdrop-blur-xl sm:px-6 xl:px-7">
            <div className="flex items-start justify-between gap-4"><div className="flex items-center gap-3"><PieChart className="h-7 w-7 text-burgundy" /><div><h2 className="text-[1.75rem] font-semibold tracking-[-.035em] text-burgundy">Carteira {selectedPortfolio}x · {monthLong(selectedMonth)}</h2><p className="mt-0.5 text-[11px] text-clay/55">Clientes com {selectedRule.parcelasNecessarias}ª parcela prevista para este mês</p></div></div><button type="button" onClick={() => setSelectedPortfolio(null)} className="rounded-lg p-2 text-burgundy/70 transition hover:bg-blush"><X className="h-5 w-5" /></button></div>
          </div>
          <div className="p-5 sm:p-6 xl:p-7">
            <div className="rounded-xl border border-[#f0c9cd] bg-[#fff0f1] p-4"><div className="flex gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-burgundy text-white"><CircleAlert className="h-4 w-4" /></span><div><p className="text-[13px] font-semibold text-burgundy">Regra do plano {selectedPortfolio}x: {selectedRule.percentual}% pago = {selectedRule.parcelasNecessarias}ª parcela</p><p className="mt-1 text-[11px] text-[#614b52]">Entram nesta carteira apenas clientes cuja <strong>{selectedRule.parcelasNecessarias}ª parcela vence em {monthLong(selectedMonth)}</strong>.</p><p className="mt-1 text-[10px] italic text-clay/45">Exemplo: se a cliente pagou {selectedRule.parcelasNecessarias - 1} parcelas até o mês anterior, ela entra nesta previsão no vencimento da {selectedRule.parcelasNecessarias}ª parcela.</p></div></div></div>

            <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4"><div className="rounded-xl border border-[#eadedf] bg-white p-3"><p className="text-[9px] text-clay/50">Clientes na carteira</p><p className="mt-1 text-xl font-semibold text-burgundy">{rawPortfolioClients.length}</p></div><div className="rounded-xl border border-[#eadedf] bg-white p-3"><p className="text-[9px] text-clay/50">Valor previsto</p><p className="mt-1 text-xl font-semibold text-burgundy">{rawPortfolioClients.reduce((sum, client) => sum + Number(client.valorCarta || 0), 0) > 0 ? formatarMoeda(rawPortfolioClients.reduce((sum, client) => sum + Number(client.valorCarta || 0), 0)) : "—"}</p></div><div className="rounded-xl border border-[#eadedf] bg-white p-3"><p className="text-[9px] text-clay/50">Mês filtrado</p><p className="mt-1 text-[15px] font-semibold text-burgundy">{monthLong(selectedMonth)}</p></div><div className="rounded-xl border border-[#eadedf] bg-white p-3"><p className="text-[9px] text-clay/50">Parcela necessária</p><p className="mt-1 text-xl font-semibold text-burgundy">{selectedRule.parcelasNecessarias}ª</p></div></div>

            <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><h3 className="flex items-center gap-2 text-[16px] font-semibold text-burgundy"><UsersRound className="h-5 w-5" />Clientes da carteira {selectedPortfolio}x</h3><label className="relative"><Search className="absolute left-3 top-2.5 h-3.5 w-3.5 text-clay/35" /><input value={portfolioSearch} onChange={(event) => { setPortfolioSearch(event.target.value); setPortfolioPage(1); }} placeholder="Buscar cliente, campanha ou vendedora..." className="h-9 min-w-[280px] rounded-lg border border-[#eadedf] bg-white pl-8 pr-3 text-[10px] outline-none focus:border-burgundy/25" /></label></div>
            <div className="mt-3 overflow-x-auto rounded-xl border border-[#eadedf]"><table className="min-w-[980px] w-full text-left"><thead className="bg-[#fbf8f7]"><tr className="text-[9px] font-semibold text-clay/55"><th className="px-3 py-2.5">Cliente</th><th className="px-3 py-2.5">Campanha</th><th className="px-3 py-2.5">Vendedora</th><th className="px-3 py-2.5">Parcelas pagas</th><th className="px-3 py-2.5">Parcela-alvo</th><th className="px-3 py-2.5">Vencimento da {selectedRule.parcelasNecessarias}ª</th><th className="px-3 py-2.5">Valor da carta</th><th className="px-3 py-2.5">Status</th><th /></tr></thead><tbody className="divide-y divide-[#f0e7e6]">{pagedPortfolioClients.map((client) => { const meta = statusMeta(client.situacao); return <tr key={client.clienteId} className="cursor-pointer text-[9px] transition hover:bg-[#fff8f7]" onClick={() => setSelectedClient(client)}><td className="px-3 py-3"><span className="font-semibold text-[#3b2d31] hover:text-burgundy">{client.nome}</span></td><td className="px-3 py-3">{client.campanha || "Sem campanha"}</td><td className="px-3 py-3">{client.responsavel || "—"}</td><td className="px-3 py-3 font-medium">{client.parcelasPagas}/{client.totalParcelas} pagas</td><td className="px-3 py-3 font-medium text-burgundy">{client.parcelasNecessarias}ª parcela</td><td className="px-3 py-3 font-medium">{dateLabel(client.previsao)}</td><td className="px-3 py-3 font-semibold">{money(client.valorCarta)}</td><td className="px-3 py-3"><span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 font-semibold ${meta.className}`}>{client.situacao === "em_risco" ? <CircleAlert className="h-3 w-3" /> : <Check className="h-3 w-3" />}{meta.label}</span></td><td className="px-2"><MoreVertical className="h-4 w-4 text-clay/45" /></td></tr>; })}{portfolioClients.length === 0 ? <tr><td colSpan={9} className="px-4 py-12 text-center text-[11px] text-clay/45">Nenhuma cliente deste parcelamento está prevista para {monthLong(selectedMonth)}.</td></tr> : null}</tbody></table></div>
            <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><p className="text-[9px] text-clay/50">Mostrando {portfolioClients.length ? (portfolioPage - 1) * pageSize + 1 : 0}–{Math.min(portfolioClients.length, portfolioPage * pageSize)} de {portfolioClients.length} clientes da carteira</p><div className="flex items-center gap-1"><button type="button" onClick={() => setPortfolioPage((page) => Math.max(1, page - 1))} className="rounded-md border border-[#eadedf] p-1"><ChevronLeft className="h-3.5 w-3.5" /></button>{Array.from({ length: Math.min(5, portfolioPages) }, (_, index) => index + 1).map((page) => <button type="button" key={page} onClick={() => setPortfolioPage(page)} className={`h-7 w-7 rounded-md border text-[9px] ${portfolioPage === page ? "border-burgundy bg-burgundy text-white" : "border-[#eadedf] bg-white text-burgundy"}`}>{page}</button>)}<button type="button" onClick={() => setPortfolioPage((page) => Math.min(portfolioPages, page + 1))} className="rounded-md border border-[#eadedf] p-1"><ChevronRight className="h-3.5 w-3.5" /></button></div></div>
