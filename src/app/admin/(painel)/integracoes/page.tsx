...
export default function IntegracoesAdminPage() {
  ...
  /** Prepara a candidata do dia. Publicar continua sendo uma ação humana explícita. */
  async function mensagemDeHoje() {
    ...
    try {
      ...
      const body = await response.json().catch(() => ({})) as { texto?: string; origem?: string; reutilizada?: boolean; motivo?: string; status?: string; aguardandoAprovacao?: boolean; erro?: string };
      ...
      const detalhe = response.ok && body.texto
        ? body.status === "aguardando_aprovacao"
          ? `Sugestão preparada e aguardando aprovação (${origem}): ${body.texto.replace(/\*/g, "")}`
          : body.status === "pronta"
            ? `Mensagem de hoje já publicada (${origem}): ${body.texto.replace(/\*/g, "")}`
            : body.status === "falha_geracao"
              ? `O Gemini não conseguiu preparar a mensagem. Nada foi publicado${body.motivo ? `: ${body.motivo}` : "."}`
              : `Mensagem em preparação: ${body.texto.replace(/\*/g, "")}`
