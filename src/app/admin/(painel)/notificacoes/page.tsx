...
export default function AdminNotificacoes() {
  ...
  const templatesVencer = useMemo(() => templates.filter((t) => t.tipo === 'parcela_vencer').sort((a, b) => (b.dias_referencia ?? 0) - (a.dias_referencia ?? 0)), [templates]);
  ...
  async function executarVencer() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.erro || 'Falha na execução.');
      setFeedback({ type: 'ok', text: `${data.enviadas ?? 0} lembrete(s) de vencimento (D-2/D-1/D0) enviado(s), ${data.ignoradas ?? 0} já registrados.` });
      await carregar();
    } catch (e: any) { setFeedback({ type: 'error', text: e.message }); }
    finally { setRunningVencer(false); }
  }
      ...
      {loading ? <div className={`${card} p-8 text-center text-sm text-clay/50 dark:text-pearl/45`}>Carregando painel...</div> : (
        <>
          {tab === 'visao' && <section className="grid gap-4 md:grid-cols-4">
            <div className={`${card} p-4`}><p className="text-[11px] uppercase tracking-wider text-clay/45 dark:text-pearl/40">Vencem em até 2 dias</p><p className="mt-2 text-2xl font-semibold text-burgundy dark:text-pearl">{aVencer}</p><p className="mt-1 text-xs text-clay/45 dark:text-pearl/40">régua D-2/D-1/D0</p></div>
          ...
          {tab === 'automacao' && <section className="grid gap-4 lg:grid-cols-[1.2fr_.8fr]">
            ...
            <div className={`${card} p-5`}><h3 className="font-semibold text-burgundy dark:text-pearl">Execução</h3><p className="mt-1 text-xs text-clay/50 dark:text-pearl/45">Use a verificação normal para respeitar o intervalo ou force um envio imediato para todas as clientes elegíveis.</p><div className="mt-5 grid gap-2"><button onClick={executarVencer} disabled={running || sendingAll || runningVencer} className="flex w-full items-center justify-center gap-2 rounded-xl border border-burgundy/15 bg-burgundy/[.04] px-4 py-3 text-xs font-semibold text-burgundy hover:bg-blush disabled:opacity-50 dark:border-white/10 dark:bg-white/[.04] dark:text-pearl"><Clock3 className="h-4 w-4" />{runningVencer ? 'Verificando...' : 'Verificar régua D-2/D-1/D0'}</button><button onClick={executarAgora} disabled={running || sendingAll || runningVencer} className="flex w-full items-center justify-center gap-2 rounded-xl border border-burgundy/15 bg-burgundy/[.04] px-4 py-3 text-xs font-semibold text-burgundy hover:bg-blush disabled:opacity-50 dark:border-white/10 dark:bg-white/[.04] dark:text-pearl"><Play className="h-4 w-4" />{running ? 'Verificando...' : 'Verificar atrasos agora'}</button><button onClick={enviarAgoraTodas} disabled={running || sendingAll || atrasadas === 0} className="flex w-full items-center justify-center gap-2 rounded-xl bg-burgundy px-4 py-3 text-xs font-semibold text-pearl shadow-soft hover:opacity-95 disabled:cursor-not-allowed disabled:opacity-50"><Send className="h-4 w-4" />{sendingAll ? 'Enviando para todas...' : 'Enviar agora para todas com atraso'}</button></div><p className="mt-3 text-[11px] leading-relaxed text-clay/45 dark:text-pearl/40">A régua D-2/D-1/D0 envia lembretes antes do vencimento; parcela paga entre execuções simplesmente para de aparecer nas próximas verificações (cancelamento automático, sem lembrete pendente a remover).</p><div className="mt-4 rounded-xl bg-burgundy/[.035] p-3 text-xs text-clay/55 dark:bg-white/[.03] dark:text-pearl/45"><strong className="text-clay dark:text-pearl">Automático:</strong><br />o worker roda a cada 30 minutos (atrasos + régua de vencimento) e respeita o intervalo configurado para cada parcela.</div></div>
          </section>}

          {tab === 'templates' && <section className="space-y-3">
            <p className="text-xs font-semibold uppercase tracking-[.14em] text-clay/45 dark:text-pearl/40">Régua de vencimento (D-2 / D-1 / D0)</p>
            {templatesVencer.map(t => { const edit = templateEdit[t.id]; const rotulo = t.dias_referencia === 0 ? 'Vence hoje (D0)' : `D-${t.dias_referencia}`; return <div key={t.id} className={`${card} p-4`}><div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between"><div className="flex min-w-0 items-start gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-blush/60 text-lg">{edit?.emoji ?? t.emoji ?? '🔔'}</span><div><div className="flex flex-wrap items-center gap-2"><h3 className="text-sm font-semibold text-burgundy dark:text-pearl">{rotulo}</h3><span className="rounded-full border border-burgundy/10 px-2 py-0.5 text-[10px] text-clay/45 dark:border-white/10 dark:text-pearl/40">antes do vencimento</span></div><p className="mt-1 text-xs text-clay/45 dark:text-pearl/40">Você pode usar: {'{{cliente}}'}, {'{{parcela}}'}, {'{{total}}'}, {'{{valor}}'}, {'{{vencimento}}'}</p></div></div>{!edit && <button onClick={() => iniciarEdicao(t)} className="rounded-xl border border-burgundy/10 px-3 py-2 text-xs font-medium text-burgundy hover:bg-blush dark:border-white/10 dark:text-pearl">Editar</button>}</div>{edit && <div className="mt-4 grid gap-3"><div className="grid gap-3 sm:grid-cols-[70px_1fr]"><input className={input} value={edit.emoji} onChange={e => setTemplateEdit({ ...templateEdit, [t.id]: { ...edit, emoji: e.target.value } })} /><input className={input} value={edit.titulo} onChange={e => setTemplateEdit({ ...templateEdit, [t.id]: { ...edit, titulo: e.target.value } })} /></div><textarea className={`${input} min-h-[100px] resize-y`} value={edit.corpo} onChange={e => setTemplateEdit({ ...templateEdit, [t.id]: { ...edit, corpo: e.target.value } })} /><div className="flex gap-2"><button onClick={() => salvarTemplate(t)} className="inline-flex items-center gap-2 rounded-xl bg-burgundy px-3 py-2 text-xs font-semibold text-pearl"><Save className="h-4 w-4" />Salvar</button><button onClick={() => setTemplateEdit(({ [t.id]: _, ...rest }) => rest)} className="rounded-xl border border-burgundy/10 px-3 py-2 text-xs text-clay dark:border-white/10 dark:text-pearl">Cancelar</button></div></div>}</div>; })}
            <p className="pt-2 text-xs font-semibold uppercase tracking-[.14em] text-clay/45 dark:text-pearl/40">Cobrança de atraso</p>
