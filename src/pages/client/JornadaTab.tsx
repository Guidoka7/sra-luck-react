      ...
      <section className="rounded-[20px] border border-rose/12 bg-white/85 p-4 shadow-card">
        ...
        <ol className="mt-4 space-y-0">
          {passos.map((passo, indice) => (
            <li key={passo.id} className="relative flex gap-3 pb-5 last:pb-0">
              ...
              <span
                className={cn(
                  "relative z-10 flex h-7 w-7 flex-none items-center justify-center rounded-full border-2",
              ...
              >
                {passo.status === "done" ? <Check className="h-3.5 w-3.5" strokeWidth={3} /> : <passo.icon className="h-3.5 w-3.5" strokeWidth={2} />}
              </span>
              <div className="min-w-0 flex-1 pt-0.5">
                <p className={cn("font-heading text-[0.85rem] font-semibold leading-tight", passo.status === "upcoming" ? "text-clay/45" : "text-burgundy")}>
                  {passo.title}
                </p>
                <p className="mt-0.5 text-[0.72rem] leading-relaxed text-clay/55">{passo.description}</p>
              </div>
            </li>
          ))}
        </ol>
