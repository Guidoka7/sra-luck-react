...
export function AgendaPage() {
  ...
  async function agendar(dataId: string) {
    setAction(dataId);
    setErro(null);
    try {
      await apiJson("/api/cliente/agendar", { method: "POST", body: JSON.stringify({ dataId, horario: "09:00" }) });
      await carregar(true);
    } catch (error) {
      setErro(error instanceof Error ? error.message : "Não foi possível confirmar a data.");
  ...
  async function enviarComprovante(event: FormEvent<HTMLInputElement>, boletoId: string) {
    ...
    setAction(`upload:${boletoId}`);
    setErro(null);
    try {
      const form = new FormData();
      form.append("arquivo", file);
      await apiJson(`/api/cliente/boletos/${encodeURIComponent(boletoId)}/anexar`, { method: "POST", body: form });
      await carregar(true);
    } catch (error) {
      setErro(error instanceof Error ? error.message : "Não foi possível enviar o comprovante.");
    } finally { setAction(null); }
  }

  async function abrirArquivo(boletoId: string, tipo: "arquivo" | "comprovante") {
    window.location.assign(`/api/cliente/boletos/${encodeURIComponent(boletoId)}/${tipo}`);
  }
  ...
  return (
    <main className="client-app min-h-screen bg-bloom px-4 pb-8 pt-4 sm:px-6 sm:pt-8">
      <div className="mx-auto w-full max-w-2xl">
        <header className="mb-5 flex items-center justify-between">
          <div className="flex items-center gap-3"><img src="/brand/sra-luck-mark.png" alt="" className="h-10 w-10" /><div><p className="text-[0.58rem] uppercase tracking-label text-rose">Bem-vinda,</p><h1 className="text-lg font-semibold text-burgundy">{firstName(agenda.cliente.nome)}</h1></div></div>
          <button onClick={() => void sair()} className="rounded-full px-3 py-2 text-xs uppercase tracking-label text-clay/50 hover:bg-white/60">Sair</button>
        </header>

        <section className="surface-glass mb-4 rounded-3xl p-4 sm:p-5">
          <div className="flex items-end justify-between"><div><p className="text-[0.62rem] uppercase tracking-label text-rose">Jornada financeira</p><p className="mt-1 text-2xl font-semibold text-burgundy">{boletos.porcentagem_pagamento.toFixed(0)}%</p></div><span className="text-xs text-clay/55">{liberada ? "Agenda liberada" : "Em acompanhamento"}</span></div>
          <div className="mt-3 h-2 overflow-hidden rounded-full bg-blush"><div className="h-full rounded-full bg-burgundy transition-all" style={{ width: `${Math.min(100, Math.max(0, boletos.porcentagem_pagamento))}%` }} /></div>
        </section>
