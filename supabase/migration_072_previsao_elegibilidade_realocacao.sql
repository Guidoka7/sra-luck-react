-- migration_072_previsao_elegibilidade_realocacao.sql
--
-- Garante que "suspender parcela" realmente a realoque para o fim do
-- cronograma sem apagar alterações manuais de vencimento das demais parcelas.
-- A previsão de elegibilidade passa a poder confiar no cronograma persistido.
...
begin
  ...
  -- Data-base: a última parcela que continua ativa. Alterações manuais de
  -- vencimento nessas parcelas são preservadas integralmente.
  ...
  -- Libera a numeração das parcelas abertas sem tocar nas parcelas pagas.
  update public.boletos
     set numero_parcela = -abs(numero_parcela),
         total_parcelas = v_total
   where cliente_id = p_cliente_id
     and status <> 'pago';

  -- Reatribui os menores números livres às parcelas abertas:
  -- primeiro as ativas, por último todas as suspensas.
  for v_boleto in
    select id, suspensa
      from public.boletos
     where cliente_id = p_cliente_id
