-- ============================================================================
-- migration_016_seguranca_concorrencia.sql
...
as $$
declare v login_rate_limits%rowtype;
begin
  select * into v from login_rate_limits where chave = p_chave for update;
  if not found then insert into login_rate_limits (chave) values (p_chave); return true; end if;
  ...
  if v.janela_iniciada_em <= now() - make_interval(secs => p_janela_segundos) then
    update login_rate_limits set falhas = 0, janela_iniciada_em = now(), bloqueado_ate = null, updated_at = now() where chave = p_chave;
    return true;
  end if;
...
returns boolean language plpgsql security definer set search_path = public
as $$
declare v login_rate_limits%rowtype; v_falhas int;
begin
  select * into v from login_rate_limits where chave = p_chave for update;
  if not found then insert into login_rate_limits (chave, falhas) values (p_chave, 1); return true; end if;
  if v.janela_iniciada_em <= now() - make_interval(secs => p_janela_segundos) then
    update login_rate_limits set falhas = 1, janela_iniciada_em = now(), bloqueado_ate = null, updated_at = now() where chave = p_chave;
    return true;
  end if;
  v_falhas := v.falhas + 1;
  update login_rate_limits set falhas = v_falhas, bloqueado_ate = case when v_falhas >= p_max_falhas then now() + make_interval(secs => p_janela_segundos) else bloqueado_ate end, updated_at = now() where chave = p_chave;
...
as $$ delete from login_rate_limits where chave = p_chave; $$;
...
create or replace function agendar_cirurgia_data(p_agendamento_id uuid, p_data date)
returns void language plpgsql security definer set search_path = public
as $$
declare v_data datas_liberacao_financeira%rowtype; v_ocupadas int;
begin
  select * into v_data from datas_liberacao_financeira where data = p_data for update;
  if not found or v_data.status <> 'disponivel' then
    raise exception using errcode = 'P0010', message = 'DATA_CIRURGIA_INDISPONIVEL';
  end if;
  select count(*)::int into v_ocupadas from agendamentos where status = 'confirmado' and previsao_liberacao_financeira = p_data and id <> p_agendamento_id;
  if v_ocupadas >= 1 then
    raise exception using errcode = 'P0011', message = 'DATA_CIRURGIA_OCUPADA';
  end if;
  update agendamentos set previsao_liberacao_financeira = p_data where id = p_agendamento_id and status = 'confirmado';
